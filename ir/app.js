...
for(const [id,key] of Object.entries({title:'title',reset:'reset',send:'send','sources-title':'sourceTitle','source-note':'sourceNote','tip-title':'tipTitle','tip-description':'tipDescription','wallet-label':'walletLabel','verify-wallet':'verify','team-link':'team',privacy:'privacy','footer-note':'footer',copy:'copy'})) $(id).textContent=t[key];
...
function renderText(element,text){element.replaceChildren();const pattern=/\[([^\]\n]+)\]\((https:\/\/[^\s)]+)\)/g;let start=0;for(const match of text.matchAll(pattern)){element.append(document.createTextNode(text.slice(start,match.index)));const a=document.createElement('a');a.textContent=match[1];a.href=safeLink(match[2])||'#';a.target='_blank';a.rel='noopener noreferrer';element.append(a);start=match.index+match[0].length;}element.append(document.createTextNode(text.slice(start)));}
