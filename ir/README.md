...
The chat helps investors and communities understand products, assess evidence and prepare questions for the founder. The founder has withdrawn the referral proposal: IR offers no intermediary commissions or creator-fee rewards. The UI contains no referral panel, referral prompts or signature flow. Grok must not revive the canceled proposal from older conversations or sources. It can explain gradual purchases and DCA in general terms, without promising returns, a lower entry price or personalized trading advice. Purchasing tokens remains an independent user decision; the chat does not execute purchases.
...
History exists only in the open browser tab and is sent with each question. No application database or chat log is created. Messages are processed by Vercel AI Gateway and either xAI (primary) or Google Vertex AI hosting Grok (fallback), under those providers' policies; xAI response storage is disabled with `store:false`. Application error logs contain stage, error type, source failure status and timeout status, not questions, answers or credentials.

## Runtime and publication

Uses Node 24, AI SDK 7 and the live Gateway catalog's `spacexai/grok-4.6` model. `IR_MODEL` may override it with a compatible xAI-hosted Grok model. If Gateway restricts the primary model to paid credits, or the provider is overloaded/unavailable, the server tries `spacexai/grok-4.1-fast-non-reasoning` on Vertex once, before any answer text has been sent. This fallback retains official source retrieval and documentation tools, but cannot search X; the assistant must disclose that limitation for X questions, and the response indicator must not claim an X search occurred. Authentication, invalid requests, cancellation and partially streamed answers do not trigger a fallback. Both attempts share the existing 52-second deadline. No credits are purchased automatically. Restore paid Gateway access to enable the primary model's X search.

AI Gateway authenticates using project-scoped Vercel OIDC at runtime; no key is sent to the browser. For local development, obtain a current project OIDC environment file via `vercel env pull`, then use `node --env-file=.env.local server.mjs`. Never commit environment files.
