...
5. The merge triggers the existing production build. After review, the maintainer copies the approved release into `scan/data/projects.json` with `approved: true` and `published: true`. The gallery and `/projects.json` feed read this registry, never incoming proposals. A proposal update alone does not change a public listing. Wait for the `Vercel – scan-devfridge-cool` deployment to succeed; visit https://hackathon.devfridge.cool/projects and verify the card/links and matching feed entry at https://hackathon.devfridge.cool/projects.json. Approval without merge, a PR preview or a failed production build is not publication.
...
The public feed at `/projects.json` has `{ "schemaVersion": 1, "projects": [...] }` and includes only records where `approved === true && published === true`, including Fridge Run. It supports public read-only CORS and revalidation on reuse. `scan/data/projects.json` is the shared source for both gallery and feed: independent snapshots of the reviewed release, source commit and evidence. Invalid records, non-boolean flags and duplicate slugs/play URLs fail validation. Follow the skill's maintainer procedure when updating it. The flags do not replace owner review or branch protection; the feed is not a continuous uptime monitor for external games.
...
- Removal: set `published: false` or remove the entry in `scan/data/projects.json` through a reviewed PR; the next successful deployment removes it from both gallery and feed. Deleting only a proposal does not remove its published snapshot. Revert the merge to roll back erroneous metadata.
