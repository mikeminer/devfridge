...
World: https://world.devfridge.cool hosts Cold Storage, a single-player brainrot merge game with a walkable kitchen. The public opening is scheduled for 1 October 2026, 00:00 Europe/Rome. Android tester beta 0.3.1-beta.1 is available as a signed APK and in the Solana dApp Store. Access requires Solana wallet sign-in and at least 500,000 tokens of one of the ten supported character mints actively timelocked in DevFridge, checked through sdk.devfridge.cool. Same-mint active locks in the connected wallet add together; different mints do not combine. Ordinary runs are free with no per-run fee or automatic payout. Optional TopShelf on Robinhood Chain lets users pay an owner-configured token fee to register a verified score; top scores can claim tokens only after the owner approves and funds a seasonal distribution. Rewards are not guaranteed. Fridge visits use saved snapshots; progress and reactions are browser-local, with no PvP or live multiplayer. Share photo & score prepares a fridge image, caption and World link for social apps, with download/copy fallback. Current rules and controls: ${DOCS_ORIGIN}/world.
