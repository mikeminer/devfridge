...
The daily refresh collects finalized Fridge program accounts directly from Solana RPC and checks the known PASTA pool's LP mint separately. Open-account counts exclude claimed/closed accounts; distinct depositors are wallets, not verified independent adopters. LP supply is dated evidence, not a guarantee of future liquidity. Failed observations retain dated evidence marked stale, never a fabricated zero. The published stats metric `pastaBurned` is labelled as an incinerator balance from its implementation; neither that balance nor the current boost vault balance establishes cumulative fee revenue. These records appear in the protocol knowledge note and in the same HTML, Markdown and JSON pitch.
