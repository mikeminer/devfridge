import { SDK_PAGES, SDK_ORIGIN } from "@/lib/sdk";

export default function SdkShell({
  kicker,
  title,
  children,
}: {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 sm:py-12">
      <p className="mb-6 text-xs font-bold tracking-[0.22em] text-ice">SDK.DEVFRIDGE.COOL</p>
      <div className="grid gap-8 lg:grid-cols-[200px_1fr]">
        <nav className="ice-card h-fit p-4 text-sm">
          <p className="mb-3 text-[10px] font-bold tracking-[0.2em] text-ice">SDK</p>
          <ul className="grid gap-1">
            {SDK_PAGES.map((p) => (
              <li key={p.href}>
                <a
                  className="block rounded-lg px-2 py-1.5 text-mute hover:bg-white/5 hover:text-ice"
                  href={`${SDK_ORIGIN}${p.href === "/" ? "" : p.href}`}
                >
                  {p.nav}
                </a>
              </li>
