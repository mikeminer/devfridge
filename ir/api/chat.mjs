...
export default async function chat(req,res){
 ...
 }catch(error){console.error('IR request failed',{stage,name:error.name,status:error.statusCode,causeName:error.cause?.name,sourceError:stage==='sources'?error.cause?.message:undefined,sourceCode:stage==='sources'?error.cause?.cause?.code:undefined,oidcAvailable:Boolean(process.env.VERCEL_OIDC_TOKEN),aborted:abort.signal.aborted});if(!res.headersSent){res.statusCode=503;res.setHeader('Content-Type','application/json');res.end(JSON.stringify({error:'temporarily_unavailable'}));}else{send({type:'error'});res.end();}}
